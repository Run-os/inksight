...
// Mixed CJK + ASCII text. Baseline grid is a 16x16 CJK cell; ASCII glyphs are
// scaled and bottom-aligned within that cell (default asciiScale=2 -> 10x14).
// CJK codepoints not in cjk16.h render as a hollow box.
void drawMixed(int x, int y, const char *text, int asciiScale) {
    ...
    while (*p) {
        if (*p < 0x80) {                                   // ASCII
            int top = (16 - 7 * asciiScale > 0) ? (16 - 7 * asciiScale) : 0;
            drawAsciiChar((char)*p, cx, y + top, asciiScale);
            cx += 5 * asciiScale + 1;                      // glyph + 1px gap
            p += 1;
...
int measureMixed(const char *text, int asciiScale) {
    ...
    const unsigned char *p = (const unsigned char *)text;
    while (*p) {
        if (*p < 0x80) { cx += 5 * asciiScale + 1; p += 1; }

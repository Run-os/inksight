# Translate all log_printf format strings in main.cpp to Chinese.
# Strategy: read text lines, use regex to find format strings inside log_printf(),
# apply translation map, write back.
import re, sys

with open('../src/main.cpp', 'r', encoding='utf-8') as f:
    content = f.read()

# Translation map: old English substring -> new Chinese substring (both between quotes)
# Only replace within log_printf("…") calls
translations = {
    # LIVE
    ...
    # '重新配网 -> portal' is already Chinese
...
for eng, chn in translations.items():
    # Match inside log_printf("…"  — the English text is part of the format string
    old_bytes = eng.encode('utf-8')
    new_bytes = chn.encode('utf-8')
    n = content.count(eng)  # count as Unicode string
    if n > 0:
        content = content.replace(eng, chn)
        count += n
    else:
        print(f'WARN: not found: {eng[:60]}')
